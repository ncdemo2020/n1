...
def updateASG(elb_name,iam_instanceprofile_name,deployment_group_name,application_name):
    ...
    try:
        ...
        current_elb = (client_elb.describe_load_balancers(LoadBalancerNames=[elb_name]))
        instanceId = (current_elb['LoadBalancerDescriptions'][0]['Instances'][0]['InstanceId'])
        print(instanceId)
        anInstance = client_ec2.describe_instances(InstanceIds=[instanceId])
        sshkeyname = anInstance['Reservations'][0]['Instances'][0]['KeyName']
        instanceType = anInstance['Reservations'][0]['Instances'][0]['InstanceType']
        instanceSecGrps = anInstance['Reservations'][0]['Instances'][0]['SecurityGroups']
        instanceIAMprofileArn = anInstance['Reservations'][0]['Instances'][0]['IamInstanceProfile']['Arn']
        instanceIAMprofile = ((instanceIAMprofileArn.split('/'))[1])
        secGroupIDs = []
        for group in instanceSecGrps:
            print(group['GroupId'])
            secGroupIDs.append(group['GroupId'])
        print(secGroupIDs)
        response = client_autoscaling.create_launch_configuration(
            ...
            IamInstanceProfile=instanceIAMprofile)
